...
module gearsbyteethanddistance(t1=13,t2=39, d=40, teethtwist=.75, which=1)
{
	cp = 360 * d/ (t1+t2);
	...
	else
	{
		...
		union()
		{
			...
			translate([0,0,gear_h+gear_shaft_h-gear2_setscrew_offset])
				...
				cylinder(r=gear2_setscrew_r, h=gear2_shaft_outer_d + 50);
			...
			//difference(){
				...
				translate([0,0,gear_h+AT])cylinder(h=gear_shaft_h+ST, r=200);
			...
			translate([0,0,-TT]) cylinder(r=gear2_bolt_hex_r, h=gear_h+gear_shaft_h-gear2_bolt_sink+AT + 50,$fn=6);
		}
...
else
{
	...
	//translate([-g1p_r,0,0]) rotate([0,0,($t*360/gear1_teeth)]) gearsbyteethanddistance(t1 = gear1_teeth, t2=gear2_teeth, d=distance_between_axels, which=1); //gear 1
...
module big_gear_new(transArr = [0, 0, 0]) {
	translate(transArr) {
		translate([g2p_r,0,0])  rotate([0,0,($t*360/gear2_teeth)*-1]) gearsbyteethanddistance(t1 = gear1_teeth, t2=gear2_teeth, d=distance_between_axels, which=2);
	}
}
...
/*------------------------------------------------------
 * This script brutally hacked for the INTVAL Next
 * http://sixteenmillimeter.com
 * ---------------------------------------------------*/

module big_gear_full () {
	translate([-65/2, 0, 0]) rotate ([180, 0, 0]) big_gear_new();
	translate([0, 0, -6]) {
		translate([0, 0, -5.5]) cylinder(r = 12 / 2, h = 5, center = true);
		translate([0, 0, -9.5]) cylinder(r = 7.3 / 2, h = 15, center = true);
		//translate([0, 0, -3.5]) cylinder(r = 12 / 2, h = 5, center = true);
		translate([0, 0, -3]) rotate([0, 0, 0]) cylinder(r = 8, h = 6, center = true, $fn = 6);
	}
}
